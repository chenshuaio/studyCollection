...
- **本地基础可用性：约 8.5/10。** 登录、注册、角色隔离、题目导入审核、公共题库、条件随机多题练习、持久化考试、错题、反馈和规则/在线回退报告可以形成基础闭环。
- **主设计功能完整度：约 74%。** 以主设计中的 40 个可验收功能点计算，完整实现约 24 项、部分实现约 11 项、尚未实现约 5 项；部分实现按半项计入。
- **目前不是“全部完结”。** 最大差距集中在结构化批量导入、报告历史与趋势、管理员模拟考试规则、个人题库和标准在线模型配置。
...
| JSON/CSV/XLSX/TXT/MD/PDF/DOCX 导入 | 部分实现 | `KnowledgeFileTextExtractor` 可读多格式 | JSON/CSV/XLSX 未按字段批量映射；无独立题目文件上传 API |
| 导入候选题可编辑预览 | 部分实现 | 页面能展示预览 | 预览表不可编辑题型、答案、解析、知识点和难度 |
...
| 自动化测试与本地发布 | 已实现 | Maven、Vitest、Vite build、启动/发布脚本 | 尚未加入 `vue-tsc` 和完整浏览器 E2E 套件 |

## 差距关闭顺序

1. 多格式结构化批量导入和可编辑预览。
2. 服务端答题明细、可信报告、历史趋势和强化题推荐。
3. 错题时间/题型筛选、反馈聚合与完整修订历史。
4. 管理员考试规则和模拟考试。
5. 个人题库、标准在线模型配置、`vue-tsc` 与数据库并发约束。
6. 在线部署前再拆分网关/独立服务并迁移标准 JWT/OIDC；这不是当前本地学习版的阻塞项。

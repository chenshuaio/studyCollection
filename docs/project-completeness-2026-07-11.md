...
- **本地基础可用性：约 9/10。** 登录、注册、角色隔离、结构化题目导入审核、学习资料出题、公共题库、条件随机多题练习、持久化考试、错题、反馈和规则/在线回退报告可以形成基础闭环。
- **主设计功能完整度：约 76%。** 以主设计中的 40 个可验收功能点计算，完整实现约 26 项、部分实现约 9 项、尚未实现约 5 项；部分实现按半项计入。
- **目前不是“全部完结”。** 最大差距集中在报告历史与趋势、管理员模拟考试规则、个人题库和标准在线模型配置。
...
| 学习控制台真实统计 | 部分实现 | 练习统计、错题、反馈均来自 API；正确率只以可自动评分题目为分母 | 缺少最近练习、最近考试、趋势和强化推荐 |
| 离线规则报告和在线失败回退 | 已实现 | `AiAnalysisService` | 报告请求仍由客户端提供结果样本，不能作为可信统计源 |
| 报告历史、正确率趋势、题型表现、考试成绩 | 未实现 | `learning_reports` 表未被仓库使用 | 缺少答题明细和报告持久化 |
| 强化题推荐 | 未实现 | 仅有“强化练习”链接 | 缺少根据薄弱知识点返回具体题目 |

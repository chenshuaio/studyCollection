...
- **本地基础可用性：约 9.5/10。** 登录、注册、角色隔离、结构化题目导入审核、学习资料出题、公共题库、条件随机多题练习、管理员模拟考试、个人组卷、持久化答题、错题、反馈和可信学习报告已形成完整基础闭环。
- **主设计功能完整度：约 92%。** 管理员考试规则、题型与难度联合配额随机组卷、规则发布/停用、模拟考试启动和响应式管理页面已补齐。
- **目前不是“全部完结”。** 最大差距集中在个人题库、标准在线模型配置/审计和数据库并发约束。
...
| 管理员配置模拟考试规则 | 已实现 | `ExamRuleService`、`RuleBasedExamGenerator`、`exam_rules`、`ExamRuleManagementPage.vue`；支持草稿、发布、停用、删除和精确联合配额组卷 | 无 |

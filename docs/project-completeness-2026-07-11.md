...
- **主设计功能完整度：约 82%。** 服务端逐题作答、报告历史、控制台近期活动、趋势、题型表现和强化题已补齐；部分实现仍按半项计入。
- **目前不是“全部完结”。** 最大差距集中在题目修订影响处理、管理员模拟考试规则、个人题库和标准在线模型配置。
...
1. 错题时间/题型筛选、反馈聚合与完整修订历史。
2. 管理员考试规则和模拟考试。
3. 个人题库、标准在线模型配置与数据库并发约束。
4. 在线部署前再拆分网关/独立服务并迁移标准 JWT/OIDC；这不是当前本地学习版的阻塞项。

...
- [ ] 写失败 API 测试：规则管理、发布/停用/删除、公开列表和启动模拟考试均携带登录令牌并解析统一响应。
- [ ] 写失败页面测试：管理员可编辑所有六种题型和三种难度配额，看到配额合计校验、规则状态及操作按钮；普通用户考试中心只显示已发布规则。
- [ ] 写失败路由测试：`/exam-rules/manage` 仅管理员可访问。
- [ ] 实现 TypeScript 类型和请求函数、管理员规则管理页、路由守卫与管理员侧边栏入口。
- [ ] 改造考试中心：顶部展示模拟考试名称、说明、题量、时限、知识点和配额，点击后创建会话并进入现有答题页；个人手动组卷保持可用。
- [ ] 运行 `npm test -- --run` 与 `npm run typecheck`，确认前端测试和类型检查通过。

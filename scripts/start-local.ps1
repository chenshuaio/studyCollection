...
$backendCommand = "Set-Location -LiteralPath '$backendPath'; mvn -pl local-app -am -DskipTests install; if (`$LASTEXITCODE -eq 0) { mvn -f local-app/pom.xml org.springframework.boot:spring-boot-maven-plugin:3.3.2:run }"
